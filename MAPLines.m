function varargout = MAPLines(varargin)
% MAPLINES MATLAB code for MAPLines.fig
%      MAPLINES, by itself, creates a new MAPLINES or raises the existing
%      singleton*.
%
%      H = MAPLINES returns the handle to a new MAPLINES or the handle to
%      the existing singleton*.
%
%      MAPLINES('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in MAPLINES.M with the given input arguments.
%
%      MAPLINES('Property','Value',...) creates a new MAPLINES or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before MAPLines_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to MAPLines_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help MAPLines

% Last Modified by GUIDE v2.5 21-Oct-2013 15:31:16

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @MAPLines_OpeningFcn, ...
                   'gui_OutputFcn',  @MAPLines_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before MAPLines is made visible.
function MAPLines_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to MAPLines (see VARARGIN)
...
map_max_init = max(aux_map_max);
%

for i = 1:handles.signal.n_conditions
    averaged_meps{i} = nan(size(handles.data.mepmean{i}{2}, 1), handles.signal.n_channels);
    averaged_meps_raw{i} = nan(size(handles.data.mepmean_bkp{i}{2}, 1), handles.signal.n_channels);
    for j = 1:handles.signal.n_channels
        if isempty(handles.data.mepmean{i}{j}) || isempty(handles.data.mepmax{i}{j})
            averaged_meps{i}(:,j) = nan(size(handles.data.mepmean{i}{2}, 1),1);
        else
            averaged_meps{i}(:,j) = handles.data.mepmean{i}{j};
        end
...
for i = 1:handles.signal.n_conditions
    ...
    for cols = 1:5
        line([1:size(averaged_meps{i},1)] + (cols-1)*size(averaged_meps{i},1),...
            averaged_meps{i}(:,[1:13] + (cols-1)*13)/max(max_amps) + repmat(13:-1:1,size(averaged_meps{i},1),1),...
            'parent', gca,'color', 'b', 'LineWidth', 2)
    end
    set(gca,'xlim',[0 (cols)*size(averaged_meps{i},1)],'ylim',[0 14],...
        'xtick', size(averaged_meps{i},1)*([1:cols] - 1/2),'xticklabel',[],...
...
%     scale = sprintf(' (%.2f microV/Div)', max(abs(averaged_meps{i}(:))));
...
% UIWAIT makes MAPLines wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = MAPLines_OutputFcn(hObject, eventdata, handles) 
