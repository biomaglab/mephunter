function varargout = MapHDsEMG_mono_angles(varargin)
...
% Last Modified by GUIDE v2.5 10-Jun-2014 23:32:59
...
signal_vars = {'signal_path', 'n_conditions', 'angle_stim', 'fsample',...
    'n_channels', 'patient_id','emg_side', 'signal_gain', 'ad_bits', 'raw_data'};
handles.signal = load(handles.signal_file, signal_vars{:});

% this is temporary - the side read is always R so I fixed to put side read in
% the file name
handles.signal.emg_side = signal_name(3);
...
handles.data.count = zeros(handles.signal.n_conditions, 1);
handles.data.cog_row = zeros(handles.signal.n_conditions, 1);
handles.data.cog_col = zeros(handles.signal.n_conditions, 1);

angles_names = '';

ad_range = 5.0;
conv_uv = 1000000.0;
emg_data_par = cell(1, handles.signal.n_conditions);
handles.signal.emg_mono = cell(1, handles.signal.n_conditions);
% reading raw signal
for i = 1:handles.signal.n_conditions
    emg_data_par{i} = handles.signal.raw_data{i}(:,...
    ...
    emg_data_par{i}(:, 16) = mean(emg_data_par{i}(:,[3 4 5 15 17 27 30 31]),2);
    
    % Coefficients for butterworth filter - passband - 25Hz a 400Hz
    [b,a] = butter(2,[10 500]*2/handles.signal.fsample(i));
    % Applying butterworth filter
    
    emg_data_par{i} = filtfilt(b,a,(emg_data_par{i}*ad_range*conv_uv)/(handles.signal.signal_gain(i)*2^(handles.signal.ad_bits)));
    
    % Organizing electrodes into an 13x5 array
    handles.signal.emg_mono{i} = [nan(length(emg_data_par{i}), 1) emg_data_par{i}(:, 1:11)...
        nan(length(emg_data_par{i}), 1) emg_data_par{i}(:, 12:50) nan(length(emg_data_par{i}), 1)...
        emg_data_par{i}(:, 51:end) nan(length(emg_data_par{i}), 1)];
end
...
for i = 1:handles.signal.n_conditions
    ...
    for j = 1:handles.signal.n_channels
        ...
        handles.data.xs{i} = (1:length(handles.signal.emg_mono{i}(:,j)))/handles.signal.fsample(i);
    end
end
...
export_data = [handles.signal.signal_path' num2cell(repmat(handles.signal.patient_id,[8 1]))...
    ...
    num2cell(repmat(handles.data.iz_row,[8 1]))...
    ...
    num2cell(handles.data.dantec_1_pp) num2cell(handles.data.sim_bip) num2cell(handles.data.sim_mono)...
    num2cell(handles.data.dantec_3_pp) num2cell(handles.data.seniam_3_pp) num2cell(handles.data.dantec_1_fmed)...
    num2cell(handles.data.sim_bip_fmed) num2cell(handles.data.sim_mono_fmed) num2cell(handles.data.dantec_3_fmed)...
    num2cell(handles.data.seniam_3_fmed) num2cell(handles.data.dantec_1_rms) num2cell(handles.data.sim_bip_rms)...
    num2cell(handles.data.sim_mono_rms) num2cell(handles.data.dantec_3_rms) num2cell(handles.data.seniam_3_rms)...
    num2cell(handles.data.count) num2cell(handles.data.cog_col) num2cell(handles.data.cog_row)];

headers = [{'file_name'} {'subject'} {'hemisphere'} {'angle(degrees)'}...
    {'iz(row)'} {'mean_total_pp(uV)'}...
    ...
    {'dantec_1_pp(uV)'} {'sim_bip_pp(uV)'} {'sim_mono_pp(uV)'}...
    {'dantec_3_pp(uV)'} {'seniam_3_pp(uV)'} {'dantec_1_freq(Hz)'}...
    {'sim_bip_freq(Hz)'} {'sim_mono_freq(Hz)'} {'dantec_3_freq(Hz)'}...
    {'seniam_3_freq(Hz)'} {'dantec_1_rms(uV)'} {'sim_bip_rms(uV)'}...
    {'sim_mono_rms(uV)'} {'dantec_3_rms(uV)'} {'seniam_3_rms(uV)'}...
    {'interp_chan'} {'cog_col'} {'cog_row'}];
...
tf = toc;
res = sprintf('Data exported in %.2f seconds', tf);
disp(res);
...
tf = toc;
res = sprintf('Data saved in %.2f seconds', tf);
disp(res);
...
% --- Executes on button press in button_Clear_Meps.
function button_Clear_Meps_Callback(hObject, eventdata, handles)
% hObject    handle to button_Clear_Meps (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
%     handles.hampthreshold{i, j} = [nan nan];
    update_waitbar(handles,j/handles.signal.n_channels)
end
...
for j = 1:handles.signal.n_channels
    for  i = 1:handles.signal.n_conditions
        ...
        handles.hsignal{i, j} = plot(handles.data.xs{i}, handles.signal.emg_mono{i}(:,j),...
            'Visible','off');
        ...
        if sum(handles.data.trigger{i}(:,j)) ~= 0
            ...
            for k = 1:length(aux_trigger) % Number of stimulus
                handles.hmeps{i, j}(k) = plot(handles.data.xs{i}(1:handles.data.s1(i)-handles.data.s0(i))+handles.MEPStart,...
                    handles.data.meps_mono{i}{j}{k},'-','color',colors(k,:),'Visible', 'off');
            end
            handles.hline{i, j} = line([handles.data.xs{i}(handles.data.trigger{i}(:,j))',...
                handles.data.xs{i}(handles.data.trigger{i}(:,j))'],...
                [min(handles.signal.emg_mono{i}(:,j)),...
                max(handles.signal.emg_mono{i}(:,j))],...
                'Color','g','Visible','off');
...
for i = 1:handles.signal.n_conditions
    for j = 1:size(handles.data.cluster_indices_rms{i},1)
        if ~isempty(handles.data.cluster_indices_rms{i})
            row = mod(handles.data.cluster_indices_rms{i}(j), 13);
            col = ceil(handles.data.cluster_indices_rms{i}(j)/13);
            if row == 0
                row = 13;
            end
            if isnan(handles.data.amp_rms_map{i}(row, col))
                total_amp_rms(i) = total_amp_rms(i);
            else
                total_amp_rms(i) = total_amp_rms(i) + handles.data.amp_rms_map{i}(row,col);
            end
        else
            total_amp_rms(i) = 0;
        end
    ...
    for k = 1:size(handles.data.cluster_indices_pp{i},1)
        if ~isempty(handles.data.cluster_indices_pp{i})
            row = mod(handles.data.cluster_indices_pp{i}(k), 13);
            col = ceil(handles.data.cluster_indices_pp{i}(k)/13);
            if row == 0
                row = 13;
            end
            if isnan(handles.data.amp_pp_map{i}(row,col))
                total_amp_rms(i) = total_amp_rms(i);
            else
                total_amp_pp(i) = total_amp_pp(i) + handles.data.amp_pp_map{i}(row,col);
            end
        else
            total_amp_pp(i) = 0;
        end
...
handles.data.sim_bip = zeros(handles.signal.n_conditions,1);
handles.data.sim_mono = zeros(handles.signal.n_conditions,1);
...
handles.data.sim_bip_fmed = zeros(handles.signal.n_conditions,1);
handles.data.sim_mono_fmed = zeros(handles.signal.n_conditions,1);
...
handles.data.sim_bip_rms = zeros(handles.signal.n_conditions,1);
handles.data.sim_mono_rms = zeros(handles.signal.n_conditions,1);
...
for i = 1:handles.signal.n_conditions % orientations
    ...
    for j = 1:handles.signal.n_channels % electrodes
        if ~isempty(handles.data.mepmean{i}{j})
            mep_mean_mono{i}(:,j) = handles.data.mepmean{i}{j};
        end
    end
    mep_mean_mono{i}(:,65) = 0;
...
%     sim_seniam_2(:,i) = aux_elec_3 - aux_elec_4;
    ...
    handles.data.dantec_1_pp(i) = peak2peak_amplitude(sim_dantec_1(:,i), handles.signal.fsample(i), handles.MEPStart, handles.data.xs{i});
%     handles.data.dantec_2_pp(i) = peak2peak_amplitude(sim_dantec_2(:,i), handles.signal.fsample(i), handles.MEPStart, handles.data.xs{i});
%     handles.data.seniam_2_pp(i) = peak2peak_amplitude(sim_seniam_2(:,i), handles.signal.fsample(i), handles.MEPStart, handles.data.xs{i});
    handles.data.dantec_3_pp(i) = peak2peak_amplitude(sim_dantec_3(:,i), handles.signal.fsample(i), handles.MEPStart, handles.data.xs{i});
    handles.data.seniam_3_pp(i) = peak2peak_amplitude(sim_seniam_3(:,i), handles.signal.fsample(i), handles.MEPStart, handles.data.xs{i});
    handles.data.sim_bip(i) = peak2peak_amplitude(seniam_bipolar(:,i), handles.signal.fsample(i), handles.MEPStart, handles.data.xs{i});
    handles.data.sim_mono(i) = peak2peak_amplitude(seniam_monopolar(:,i), handles.signal.fsample(i), handles.MEPStart, handles.data.xs{i});
...
%         handles.signal.fsample(i), handles.data.s1(i)-handles.data.s0(i));
    ...
    [handles.data.seniam_3_fmed(i), handles.data.seniam_3_rms(i), ~] = Fmed3cla(sim_seniam_3(:,i),...
        handles.signal.fsample(i), handles.data.s1(i)-handles.data.s0(i));
    [handles.data.sim_bip_fmed(i), handles.data.sim_bip_rms(i), ~] = Fmed3cla(seniam_bipolar(:,i),...
        handles.signal.fsample(i), handles.data.s1(i)-handles.data.s0(i));
    [handles.data.sim_mono_fmed(i), handles.data.sim_mono_rms(i), ~] = Fmed3cla(seniam_monopolar(:,i),...
        handles.signal.fsample(i), handles.data.s1(i)-handles.data.s0(i));    
end
xlswrite('bipolar_sim_6_R.xls', seniam_bipolar);
xlswrite('monopolar_sim_6_R.xls', seniam_monopolar);
tf = toc;
res = sprintf('MEP simulated in %.2f seconds', tf);
disp(res);
...
if get(handles.checkbox_ViewSignal,'Value')
    ...
    if sum(handles.data.trigger{handles.id_angle}(:,pos)) ~= 0
        ...
        handles.hsignal{handles.id_angle}(pos) = plot(handles.data.xs{handles.id_angle},handles.signal.emg_mono{handles.id_angle}(:,pos));
        % red circulus
...
%         end
        ...
        if sum(handles.data.trigger{handles.id_angle}(:,pos)) ~= 0
            handles.hline{handles.id_angle, pos} = line([handles.data.xs{handles.id_angle}(handles.data.trigger{handles.id_angle}(:,pos))',...
                ...
                [min(handles.signal.emg_mono{handles.id_angle}(:,pos)),...
                max(handles.signal.emg_mono{handles.id_angle}(:,pos))],...
                'Color','g');
        else
    ...
    out_check = handles.data.count(handles.id_angle);
    ...
    for k = 1:length(aux_trigger)
        handles.hmeps{handles.id_angle, pos}(k) = plot(handles.data.xs{handles.id_angle}(1:handles.data.s1(handles.id_angle)-handles.data.s0(handles.id_angle))+handles.MEPStart,...
            handles.data.meps_mono{handles.id_angle}{pos}{k},'-','color',colors(k,:),'Visible','off');
    end
    ...
    mepmeanvalue = get(handles.checkbox_ViewMEPsMean,'Value');
    if handles.data.count(handles.id_angle) > out_check
        plotcolor = 'k';
    else
        plotcolor = 'b';
    end
    handles.hmepmean{handles.id_angle, pos} = plot(handles.data.xs{handles.id_angle}(1:handles.data.s1(handles.id_angle)-handles.data.s0(handles.id_angle))+handles.MEPStart,...
        handles.data.mepmean{handles.id_angle}{pos}, 'Color', plotcolor , 'Visible','off');
    if mepmeanvalue == 1
        if isfield(handles,'hmepmean')
            if ishandle(handles.hmepmean{handles.id_angle, pos})
...
for j = 1:handles.signal.n_channels % Electrodes
    ...
    for i = 1:handles.signal.n_conditions % Angles
        ...
        aux_channel = handles.signal.emg_mono{i}(:,j);
        aux_meps = [];
        aux_meps_column = [];
        ...
        for k = 1:length(aux_trigger) % Number of stimulus
            handles.data.meps_mono{i}{j}{k} = aux_channel(aux_trigger(k)+handles.data.s0(i):aux_trigger(k)+handles.data.s1(i)-1);
            handles.data.offset_signals{i}{j}{k} = aux_channel(aux_trigger(k)-handles.data.s1(i)-30+1:aux_trigger(k)-handles.data.s0(i)-30);
            handles.hmeps{i, j}(k) = plot(handles.data.xs{i}(1:handles.data.s1(i)-handles.data.s0(i))+handles.MEPStart,...
                handles.data.meps_mono{i}{j}{k},'-','color',colors(k,:),'Visible', 'off');
            if k == 1
                aux_meps_column = handles.data.meps_mono{i}{j}{k};
                aux_meps = handles.data.meps_mono{i}{j}{k};
                aux_offset_signal_columns = handles.data.offset_signals{i}{j}{k};
                aux_offset_signal = handles.data.offset_signals{i}{j}{k};
            else
                aux_meps_column = cat(1, aux_meps_column, handles.data.meps_mono{i}{j}{k});
                aux_meps = cat(2, aux_meps, handles.data.meps_mono{i}{j}{k});
...
%         handles.data.meps_mono_bkp = handles.data.meps_mono;
...
if handles.hp_filter == 10 || handles.lp_filter == 500
...
if handles.hp_filter ~= 10 || handles.lp_filter ~= 500
    handles.signal.emg_mono = emg_mono_aux;
    handles.signal = rmfield(handles.signal, {'raw_data'});
end
clear emg_data_aux emg_mono_aux
...
if handles.data.slope == 0
    for j = 1:handles.signal.n_channels % Electrodes
        ...
        for i = 1:handles.signal.n_conditions % Angles
            ...
            if sum(isnan(handles.signal.emg_mono{i}(samples_triggeron{i},j)))==0
...
%             end
            ...
            if sum(handles.data.trigger{i}(:,j))~=0
                a = line([handles.data.xs{i}(handles.data.trigger{i}(:,j))',...
                    handles.data.xs{i}(handles.data.trigger{i}(:,j))'],...
                    [min(handles.signal.emg_mono{i}(:,j)),...
                    max(handles.signal.emg_mono{i}(:,j))],'Color','g','Visible','off');
                handles.hline{i, j} = a;
            else
                handles.hline{i, j} = nan;
            end
...
% this part is not fixed for the actual code
    ...
    for i = 1:length(handles.signal.emg_mono) 
        axes(eval(strcat('handles.axes',num2str(i))))           
        handles.data.trigger{i} = BiopacTrigger(handles.signal.emg_mono{i}(:,handles.id_angle),handles.data.slope);
        ...
        if ~isempty(handles.data.trigger{i})
            a = line([handles.data.xs{handles.id_angle}{i}(handles.data.trigger(:,i))',handles.data.xs{handles.id_angle}{i}(handles.data.trigger(:,i))'],...
                [min(handles.signal.emg_mono{i}(:,handles.id_angle)),...
                max(handles.signal.emg_mono{i}(:,handles.id_angle))],...
                'Color','g','Visible','off');
            handles.hline{i} = a;
        else
            handles.hline{i} = nan;
        end
        update_waitbar(handles,i/length(handles.signal.emg_mono))
    end
end
